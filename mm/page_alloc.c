				...
				unsigned int order, unsigned int alloc_flags)
{
	...
	/*
	 * If the caller does not have rights to ALLOC_HARDER, subtract the
	 * high-atomic reserves. This can overestimate the reserve, but avoids
	 * scanning the free lists in the watermark fast path.
	 */
	if (likely(!alloc_harder))
		unusable_free += z->nr_reserved_highatomic;

#ifdef CONFIG_CMA
	/* If the allocation cannot use CMA areas, exclude free CMA pages. */
		...
		unsigned long mark, int classzone_idx, unsigned int alloc_flags)
{
	...
	if (!order) {
		...
		/* The high-atomic reserve estimate may exceed available pages. */
